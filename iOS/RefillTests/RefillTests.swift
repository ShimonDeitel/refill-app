...
final class RefillTests: XCTestCase {
    ...
    func testFreeLimitBlocksAddition() {
        store.isPro = false
        for i in 0..<(RefillStore.freeLimit + 5) {
            store.add(title: "Filler \(i)", amount: 1.0)
        }
        XCTAssertEqual(store.entries.count, RefillStore.freeLimit)
    }
    ...
    func testCanAddMoreWhenPro() {
        ...
        XCTAssertGreaterThan(store.entries.count, RefillStore.freeLimit)
    }
    ...
    func testFreeLimitAboveSeedCount() {
        XCTAssertGreaterThan(RefillStore.freeLimit, 3)
    }
}
